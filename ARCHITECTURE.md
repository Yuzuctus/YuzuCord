...
Les projets, namespaces et scripts actifs portent le nom YuzuCord. L’identifiant persistant
`YuzuctusVencord` et son dossier de données restent volontairement stables afin que les installations existantes
puissent être mises à jour sans migration risquée. Les anciens dossiers RandomFavorites ne sont lus que par le
chemin de migration dédié.

Le cœur de l’installateur sépare désormais clairement :

- `InstallerService`, qui orchestre les opérations visibles par l’utilisateur ;
- `BundlePayloadStore`, qui valide, extrait, active et nettoie les versions compilées ;
- `DiscordInstallationService`, qui détecte les éditions Discord, ferme uniquement le processus choisi,
  lance l’installateur Vencord et vérifie le patch appliqué ;
- `ReleaseClient`, qui résout et télécharge les assets GitHub vérifiés.
...
Le manager source (`scripts/YuzuCordManager.ps1`) clone ou met à jour le dépôt de distribution et le Vencord
officiel, matérialise le catalogue, installe les dépendances exactes, compile puis injecte. Son état est alimenté
par `resolved-plugins.json`. Sa TUI réside dans `scripts/lib/YuzuCord.Tui.ps1`; le lanceur de mise à jour copie le
script et sa bibliothèque ensemble.
...
La même entrée est utilisée localement, dans la CI et pendant les releases :

```powershell
pwsh ./scripts/Invoke-YuzuCordValidation.ps1 `
  -Targets All `
  -VencordDirectory ../Vencord `
  -AllowLocalGitSources
```

Les cibles `Catalog`, `Vencord` et `Installer` peuvent être lancées séparément pendant le développement.

## Validation attendue

Une modification n’est considérée compatible qu’après validation du catalogue, des scripts PowerShell, des
tests TypeScript, d’ESLint, de Stylelint, du typecheck, du build Vencord courant, du build .NET et des smoke tests
de l’installeur. Les changements du manager sont aussi rejoués avec `-SkipInject` afin de tester le parcours
source-build sans modifier Discord.
