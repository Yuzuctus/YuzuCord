...
Depuis le dépôt de distribution, utilise l’entrée de validation commune à la CI et aux releases :

```powershell
pwsh ./scripts/Invoke-YuzuCordValidation.ps1 `
  -Targets Catalog,Vencord `
  -VencordDirectory C:\chemin\vers\Vencord `
  -AllowLocalGitSources
```

Cette commande valide le catalogue, matérialise les plugins, exécute leurs tests, ESLint, Stylelint,
TypeScript et construit le bundle Vencord de production.
