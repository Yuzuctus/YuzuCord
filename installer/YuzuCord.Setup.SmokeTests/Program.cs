...
using YuzuCord.Setup.Core;
using YuzuCord.Setup.Core.Models;
using YuzuCord.Setup.Core.Services;
using YuzuCord.Setup.Presentation;
...
var tests = new (string Name, Action Run)[]
{
    ...
    ("bundle extraction accepts a complete verified payload", TestBundleExtraction),
    ("bundle extraction rejects path traversal entries", TestBundleTraversalGuard),
    ...
    ("OpenAsar replaces a stale YuzuCord patcher backup", TestOpenAsarStaleYuzuCordBackup),
    ("OpenAsar reuses its owned backup after Vencord repair", TestOpenAsarOwnedBackupReuse),
    ...
    ("installer primary action stays in the persistent footer", TestInstallerPrimaryActionLayout),
...
static void TestBundleExtraction()
{
    var temporary = Path.Combine(Path.GetTempPath(), $"yuzucord-bundle-{Guid.NewGuid():N}");
    var layout = new InstallerLayout(
        Path.Combine(temporary, "local"),
        Path.Combine(temporary, "roaming"));
    layout.EnsureDirectories();
    var bundle = Path.Combine(temporary, "bundle.zip");
    var destination = Path.Combine(layout.Versions, ".staging-valid");
    var manifest = CreateCatalogManifest("v2.0.0", 'a', 'b');
    ...
    try
    {
        using (var archive = ZipFile.Open(bundle, ZipArchiveMode.Create))
        {
            ...
            WriteZipEntry(archive, "dist/patcher.js", "patcher");
            WriteZipEntry(archive, "dist/preload.js", "preload");
            WriteZipEntry(archive, "dist/renderer.js", "renderer");
            WriteZipEntry(archive, "dist/renderer.css", "styles");
            WriteZipEntry(archive, "tools/VencordInstallerCli.exe", "installer");
        }

        var store = new BundlePayloadStore(layout, _ => { });
        var extracted = store.ExtractAndValidateAsync(
                bundle,
                destination,
                CancellationToken.None)
            .GetAwaiter()
        ...
        Assert(extracted.Version == manifest.Version);
        Assert(File.Exists(Path.Combine(destination, "dist", "patcher.js")));
        Assert(File.Exists(Path.Combine(destination, "tools", "VencordInstallerCli.exe")));
    }
    finally
    {
        if (Directory.Exists(temporary))
...
static void TestBundleTraversalGuard()
{
    var temporary = Path.Combine(Path.GetTempPath(), $"yuzucord-bundle-guard-{Guid.NewGuid():N}");
    var layout = new InstallerLayout(
        Path.Combine(temporary, "local"),
        Path.Combine(temporary, "roaming"));
    layout.EnsureDirectories();
    var bundle = Path.Combine(temporary, "bundle.zip");
    var destination = Path.Combine(layout.Versions, ".staging-dangerous");

    try
    {
        using (var archive = ZipFile.Open(bundle, ZipArchiveMode.Create))
            WriteZipEntry(archive, "../outside.txt", "refused");

        var store = new BundlePayloadStore(layout, _ => { });
        AssertThrows<InvalidDataException>(() => store.ExtractAndValidateAsync(
                bundle,
                destination,
                CancellationToken.None)
            .GetAwaiter()
            .GetResult());
        Assert(!File.Exists(Path.Combine(layout.Versions, "outside.txt")));
    }
    finally
    {
        if (Directory.Exists(temporary))
...
static void WriteZipEntry(ZipArchive archive, string path, string content)
{
...
static void TestOpenAsarStaleYuzuCordBackup()
{
    var temporary = CreateDiscordFixture(withVencordPatch: false);
    var discord = CreateDiscordInstallation(temporary);
    var resources = GetFixtureResources(temporary);
    ...
    File.WriteAllText(
        ...
        "require(\"C:\\\\Users\\\\sammy\\\\AppData\\\\Local\\\\YuzuctusVencord\\\\versions\\\\v2-beta\\\\dist\\\\patcher.js\")");
    ...
    try
    {
        ...
        Assert(File.ReadAllText(Path.Combine(resources, "app.asar")) == "OpenAsar replacement");
        Assert(File.ReadAllText(Path.Combine(resources, "app.asar.backup")) == "Discord original");
        Assert(File.Exists(Path.Combine(resources, "app.asar.backup.yuzucord")));
...
static void TestOpenAsarOwnedBackupReuse()
{
    var temporary = CreateDiscordFixture(withVencordPatch: false);
    var discord = CreateDiscordInstallation(temporary);
    var resources = GetFixtureResources(temporary);
    var firstOpenAsar = Path.Combine(temporary, "openasar-first.asar");
    var nextOpenAsar = Path.Combine(temporary, "openasar-next.asar");
    File.WriteAllText(firstOpenAsar, "OpenAsar first release");
    File.WriteAllText(nextOpenAsar, "OpenAsar next release");
    ...
    try
    {
        ...
        Assert(OpenAsarManager.InstallOrUpdate(discord, nextOpenAsar) == OpenAsarChange.Installed);
        Assert(File.ReadAllText(Path.Combine(resources, "app.asar")) == "OpenAsar next release");
        Assert(File.ReadAllText(Path.Combine(resources, "app.asar.backup")) == "Discord original");
...
static void TestInstallerPrimaryActionLayout()
{
    ...
    var candidates = new[]
    {
        ...
        Path.Combine(Directory.GetCurrentDirectory(), "distribution", relativePath),
    };
    var xamlPath = candidates.FirstOrDefault(File.Exists)
        ?? throw new FileNotFoundException("MainWindow.xaml was not found for the UI layout check.");
    var document = XDocument.Load(xamlPath);
    XNamespace presentation = "http://schemas.microsoft.com/winfx/2006/xaml/presentation";
    XNamespace xaml = "http://schemas.microsoft.com/winfx/2006/xaml";

    var primaryAction = document
    ...
    var actionPanel = primaryAction
        ...
        .Single(element =>
            string.Equals(
                ...
                "ActionPanel",
                StringComparison.Ordinal));
    var footer = actionPanel
        .AncestorsAndSelf()
        .FirstOrDefault(element => element.Attributes().Any(attribute =>
            string.Equals(attribute.Name.LocalName, "Grid.Row", StringComparison.Ordinal)
            && string.Equals(attribute.Value, "2", StringComparison.Ordinal)));
    var mainScrollViewer = document
    ...
    Assert(footer is not null);
    Assert(!mainScrollViewer.DescendantsAndSelf().Contains(primaryAction));
