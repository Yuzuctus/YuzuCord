...
function Read-YuzuctusPluginCatalog {
    ...
    foreach ($plugin in $plugins) {
        ...
        Assert-AllowedProperties $plugin @(
            ...
            'provenance', 'dependencies', 'conflicts', 'license', 'licenseFile',
            'maintainer', 'status'
        ) "Plugin '$id'"

        foreach ($property in @("displayName", "entrypoint", "provenance", "license", "licenseFile", "maintainer", "status")) {
        ...
        foreach ($property in @("dependencies", "conflicts", "mappings")) {
            if (-not (Test-ObjectProperty $plugin $property)) {
                throw "Plugin '$id' is missing '$property'."
            }
        }
        ...
        if ([string]$plugin.provenance -notin @("yuzuctus", "thirdParty")) {
            throw "Plugin '$id' has an unsupported provenance '$($plugin.provenance)'."
        }
...
function Get-PluginDistributionTags {
    param([object]$Plugin)

    switch ([string]$Plugin.provenance) {
        "yuzuctus" { return "YuzuMod" }
        "thirdParty" { return "ThirdParty" }
        default { throw "Plugin '$($Plugin.id)' has an unsupported provenance '$($Plugin.provenance)'." }
    }
}
...
function Add-DistributionPluginTags {
    ...
    $tagsJson = ConvertTo-Json -InputObject @(Get-PluginDistributionTags $Plugin) -Compress
    $wrapper = @"
/*
...
// Generated by YuzuCord's plugin catalog. Do not edit the materialized copy.
    ...
    try {
        ...
        foreach ($plugin in @($catalogInfo.Plugins)) {
            ...
            $resolvedPlugins += ,[PSCustomObject][ordered]@{
                ...
                provenance = [string]$plugin.provenance
                distributionTags = @(Get-PluginDistributionTags $plugin)
